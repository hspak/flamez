...
const CpuTimeFn = *const fn (context: ?*anyopaque, pid: std.posix.pid_t, total_ns: *u64) c_int;
...
extern "c" fn flamez_macos_process_identity(pid: std.posix.pid_t, identity: *ProcessIdentity) c_int;
...
extern "c" fn flamez_macos_es_test_enqueue(collector: *EsHandle, event: *const EsEvent) c_int;
...
extern "c" fn flamez_macos_es_test_prepare_sync(collector: *EsHandle, reject: c_int) void;
...
fn systemCpuTime(_: ?*anyopaque, pid: std.posix.pid_t, total_ns: *u64) c_int {
    return flamez_macos_cpu_time(pid, total_ns);
}

fn systemIdentity(_: ?*anyopaque, pid: std.posix.pid_t, identity: *ProcessIdentity) c_int {
...
pub fn init(gpa: Allocator) Macos {
    return initWithOptions(gpa, .{
        .endpoint_security = if (comptime build_options.macos_require_endpoint_security)
            .required
        else
            .automatic,
    });
...
/// `gpa` is retained; assume it supports concurrent worker and session allocations.
pub fn initWithOptions(gpa: Allocator, options: Options) Macos {
    var self = Macos{
        .gpa = gpa,
...
pub fn deinit(self: *Macos) void {
    ...
    const result = std.c.pthread_mutex_destroy(&self.mutex);
    std.debug.assert(result == .SUCCESS);
    self.* = undefined;
}

/// Returns whether a process-event queue was created.
pub fn available(self: *const Macos) bool {
    return self.kqueue_fd >= 0;
}

/// macOS process inspection does not require a post-attach privilege drop.
pub fn dropPrivileges(_: *const Macos) capture.DropPrivilegesError!void {}

/// Returns collector-owned initialization diagnostics.
pub fn diagnosticSlice(self: *const Macos) []const u8 {
...
pub fn exactDiagnosticSlice(self: *const Macos) []const u8 {
...
pub fn fidelity(self: *const Macos) capture.Fidelity {
...
pub fn armLaunch(self: *Macos, launcher_pid: std.posix.pid_t) capture.ArmLaunchError!void {
    self.stopWorker();
    self.closeEs();
    self.lock();
    self.clearTrackedLocked();
...
/// Allocator failures propagate. Assume `self` stays at this address until the
/// capture worker stops or `deinit` returns.
pub fn trackRoot(self: *Macos, pid: std.posix.pid_t) capture.TrackRootError!void {
    if (self.kqueue_fd < 0) return;
    self.lock();
    defer self.unlock();
    if (self.es_handle != null) {
        try self.es_versions.put(self.gpa, pid, 0);
        self.root_pid = pid;
        self.root_pgid = pid;
    ...
    try self.tracked.ensureUnusedCapacity(self.gpa, 1);
    ...
    self.worker = std.Thread.spawn(.{}, workerMain, .{self}) catch |err| {
        ...
        return switch (err) {
            ...
            error.Unexpected,
            => error.LaunchTrackingRejected,
        };
    };
}

/// Removes a tracked PID and its kqueue registration.
pub fn untrack(self: *Macos, pid: std.posix.pid_t) void {
    self.lock();
    defer self.unlock();
...
pub fn pollEvents(self: *Macos, sink: capture.Sink) void {
    ...
    self.lock();
    std.mem.swap(std.ArrayList(PendingEvent), &self.pending_events, &self.delivery_events);
...
pub fn flushEvents(self: *Macos, sink: capture.Sink) void {
    if (self.es_handle) |handle| {
...
pub fn snapshotCpu(self: *Macos, sink: capture.Sink) void {
    self.snapshotCpuWith(sink, null, systemCpuTime, systemIdentity, systemTimestamp);
}
...
fn snapshotCpuWith(
    ...
    sink: capture.Sink,
    context: ?*anyopaque,
    comptime cpu_time_fn: CpuTimeFn,
    comptime identity_fn: IdentityFn,
    comptime timestamp_fn: TimestampFn,
) void {
    self.last_cpu_samples = 0;
    if (self.kqueue_fd < 0) return;
...
fn consumeEndpointSecurityEvent(self: *Macos, sink: capture.Sink, event: *const EsEvent) void {
...
fn consumeEsFork(self: *Macos, sink: capture.Sink, event: *const EsEvent, name: []const u8) void {
    self.lock();
...
fn consumeEsExec(self: *Macos, sink: capture.Sink, event: *const EsEvent, name: []const u8) void {
    self.lock();
...
fn consumeEsExit(self: *Macos, sink: capture.Sink, event: *const EsEvent, name: []const u8) void {
    self.lock();
...
fn admitEsCpuLocked(self: *Macos, pid: std.posix.pid_t, name: []const u8) void {
    var identity: ProcessIdentity = undefined;
...
fn consumeReadyEventsLocked(self: *Macos, events: []const std.c.Kevent, timestamp_ns: u64) bool {
...
fn hasTrackedForkHintLocked(self: *Macos, events: []const std.c.Kevent) bool {
    for (events) |event| {
...
fn consumeKeventLocked(self: *Macos, event: std.c.Kevent, timestamp_ns: u64) void {
...
fn emitPendingMetadataLocked(self: *Macos, timestamp_ns: u64) void {
    var iterator = self.tracked.iterator();
    while (iterator.next()) |entry| {
...
fn discoverDescendantsLocked(self: *Macos, timestamp_ns: u64) void {
...
fn listChildren(self: *Macos, parent_pid: std.posix.pid_t) ?[]const std.posix.pid_t {
    return self.listPids(.children, parent_pid);
}

fn listPids(self: *Macos, kind: PidList, identifier: std.posix.pid_t) ?[]const std.posix.pid_t {
...
fn listPidsWith(
    ...
    const max_capacity = @divTrunc(@as(usize, @intCast(std.math.maxInt(c_int))), pid_size);
...
fn discoverProcessGroupLocked(self: *Macos, timestamp_ns: u64) void {
...
fn discoverEscapedDescendantsLocked(self: *Macos, timestamp_ns: u64) void {
    ...
    while (admitted_in_pass) {
        ...
        for (self.identity_snapshot.items) |*candidate| {
            ...
            candidate.pid = 0;
            if (self.admitChildIdentityLocked(parent_pid, pid, candidate.identity, timestamp_ns)) {
...
fn trackedPidByUniqueIdLocked(self: *Macos, unique_id: u64) ?std.posix.pid_t {
    var iterator = self.tracked.iterator();
    while (iterator.next()) |entry| {
...
fn queueExecLocked(self: *Macos, pid: std.posix.pid_t, timestamp_ns: u64) void {
    _ = self.queueExecSnapshotLocked(pid, timestamp_ns);
}

fn queueExecHintLocked(self: *Macos, pid: std.posix.pid_t, timestamp_ns: u64) void {
...
fn queueExecSnapshotLocked(self: *Macos, pid: std.posix.pid_t, timestamp_ns: u64) bool {
...
fn retryMetadataLocked(self: *Macos, pid: std.posix.pid_t, generation: usize) void {
    const tracked = self.tracked.getPtr(pid) orelse return;
    if (tracked.generation == generation) tracked.needs_metadata = true;
}

fn queueExitLocked(self: *Macos, pid: std.posix.pid_t, timestamp_ns: u64) void {
    const tracked = self.tracked.get(pid) orelse return;
    var cpu_ns: u64 = 0;
    var cpu_final = false;
...
fn queueEventLocked(self: *Macos, event: PendingEvent) void {
    var owned = event;
...
fn refreshName(_: *Macos, pid: std.posix.pid_t, tracked: *Tracked) void {
...
fn register(self: *Macos, pid: std.posix.pid_t, generation: usize) bool {
...
fn deleteRegistration(self: *Macos, pid: std.posix.pid_t) void {
    if (self.kqueue_fd < 0) return;
    const change = std.c.Kevent{
        .ident = @intCast(pid),
        .filter = std.c.EVFILT.PROC,
...
fn clearTrackedLocked(self: *Macos) void {
...
fn clearPendingEventsLocked(self: *Macos) void {
...
fn newGeneration(self: *Macos) usize {
    self.next_generation +%= 1;
    if (self.next_generation == 0) self.next_generation = 1;
    return self.next_generation;
}

fn noteLossLocked(self: *Macos, message: []const u8) void {
...
fn setExactDiagnostic(self: *Macos, value: []const u8) void {
...
test "collector initializes without special privileges" {
    var collector = Macos.init(std.testing.allocator);
...
test "fallback lifecycle queue is bounded and reports overflow" {
    ...
    var collector = Macos.initWithOptions(testing.allocator, .{
...
test "fallback PID snapshots grow when the fill call reaches capacity" {
    const Probe = struct {
        ...
        fn list(
            ...
            const capacity = @divTrunc(@as(usize, @intCast(buffer_size)), @sizeOf(std.posix.pid_t));
...
test "fallback PID snapshots report persistent truncation" {
    ...
    try testing.expect(collector.listPidsWith(.all, 0, &probe, Probe.list) == null);
...
test "Endpoint Security bridge owns queued slices and preserves FIFO order" {
        ...
        fn event(pointer: ?*anyopaque, observation: *const EsEvent) callconv(.c) void {
            ...
            if (self.count == 0) {
                self.metadata_matches =
                    std.mem.eql(u8, esSlice(observation.name, observation.name_len).?, "tool") and
                    std.mem.eql(
                        u8,
                    ...
                    std.mem.eql(u8, esSlice(observation.cwd, observation.cwd_len).?, "/tmp");
            }
            self.count += 1;
        }
    };
    ...
    var probe = Probe{};
    try testing.expectEqual(@as(c_int, 2), flamez_macos_es_poll(handle, Probe.event, &probe));
...
test "Endpoint Security bridge extracts synthetic native messages" {
    const Probe = struct {
        ...
        fn event(pointer: ?*anyopaque, observation: *const EsEvent) callconv(.c) void {
            const self: *@This() = @ptrCast(@alignCast(pointer.?));
            const expected_timestamp = flamez_macos_es_test_mach_time_to_ns(self.count + 1);
            switch (observation.kind) {
                .fork => self.fork_matches =
                    ...
                    std.mem.eql(u8, esSlice(observation.name, observation.name_len).?, "child"),
                ...
                .exit => self.exit_matches =
                    ...
                    std.mem.eql(u8, esSlice(observation.name, observation.name_len).?, "tool"),
            }
            self.count += 1;
        }
    };
    ...
    var probe = Probe{};
    try testing.expectEqual(@as(c_int, 3), flamez_macos_es_poll(handle, Probe.event, &probe));
...
test "Endpoint Security bridge reports queue overflow and sequence gaps" {
    ...
    var probe = Probe{};
    try testing.expectEqual(@as(c_int, 1), flamez_macos_es_poll(handle, Probe.event, &probe));
...
test "kqueue ignores an event from an earlier PID generation" {
    ...
    const pid = process_ops.currentPid();
    var identity: ProcessIdentity = undefined;
    try std.testing.expectEqual(@as(c_int, 0), flamez_macos_process_identity(pid, &identity));
...
test "fallback recovers an escaped child by immutable parent identity" {
    ...
    const escaped_pid = try waitForPidFile(temporary.dir, testing.io, "daemon.pid", &pid_buffer);
...
test "snapshots cumulative CPU for a tracked process" {
    const Probe = struct {
        ...
        fn cpu(pointer: *anyopaque, pid: std.posix.pid_t, _: u64, timestamp_ns: u64) void {
            const self: *@This() = @ptrCast(@alignCast(pointer));
            self.calls += 1;
            self.pid = pid;
...
test "CPU snapshot timestamps each process after its cumulative read" {
    const Probe = struct {
        ...
        fn readCpu(pointer: ?*anyopaque, pid: std.posix.pid_t, total_ns: *u64) c_int {
            const self: *@This() = @ptrCast(@alignCast(pointer.?));
        ...
        fn cpu(pointer: *anyopaque, _: std.posix.pid_t, _: u64, timestamp_ns: u64) void {
            const self: *@This() = @ptrCast(@alignCast(pointer));
    ...
    const testing = std.testing;
    var collector = Macos.init(testing.allocator);
    defer collector.deinit();
...
test "CPU snapshot advances after parallel thread work" {
    const Probe = struct {
        ...
        fn cpu(pointer: *anyopaque, _: std.posix.pid_t, cpu_ns: u64, timestamp_ns: u64) void {
            const self: *@This() = @ptrCast(@alignCast(pointer));
            self.cpu_ns = cpu_ns;
            self.timestamp_ns = timestamp_ns;
        }
    ...
    const Busy = struct {
        fn run(completed_cycles: *std.atomic.Value(u64), seed: u64) void {
            var value = seed | 1;
...
test "fallback exit preserves CPU and name when a PID belongs to another lifetime" {
    const testing = std.testing;
    var collector = Macos{ .gpa = testing.allocator };
    defer collector.deinit();
    const pid = process_ops.currentPid();
    var identity: ProcessIdentity = undefined;
    try testing.expectEqual(@as(c_int, 0), flamez_macos_process_identity(pid, &identity));
...
test "fallback recovery reuses its parent queue between scans" {
    ...
    var collector = Macos{ .gpa = failing.allocator() };
    defer collector.deinit();
    const pid = process_ops.currentPid();
    var identity: ProcessIdentity = undefined;
    try testing.expectEqual(@as(c_int, 0), flamez_macos_process_identity(pid, &identity));
